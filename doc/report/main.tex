...
\begin{abstract}
%\boldmath
Autonomous warehouses play an important role in smart factory environments, which is an essential element of "Industry 4.0". The goal is to optimize the transportation of goods within the warehouse using multi-robot collaboration and coordination. This work uses hybrid task planning to improve the task assignment decisions and distributing the computational effort between the central task planner and the decentralized agents. A decentralized path planning with timed reservations, using a shared map, ensures the coordination between all robots while moving. The results show that this approach yields a fast and reliable way of transporting goods in a warehouse and remains efficient, up to a relatively high density of robots. Due to the limited space, an increasing amount of robots leads to congestions as well as coordination overhead, reducing the overall efficiency at some point. This point can be determined within our simulation to find the best suitable amount of robots for a given warehouse configuration.
\end{abstract}
...
The performance of our system was evaluated on two different maps (Table \ref{tab:maps}) of size 18x16 using different number of robots (1, 2, 4, 6, 8, 10 and 12) with the same characteristics. The simulation was repeated 3 times for each number of robots on each map. It was declared as finished when 40 transportation tasks were completed. The initial battery level of the robots is randomized between 95 and 100.

For transportation tasks we sampled a) the timestamp when the task is assigned, b) the execution time of the task, c) the time for driving to pickup, d) the pickup time, e) the time for driving to drop-off, f) the drop-off time and g) the battery level at the end of each task. For charging tasks we sampled a) the timestamp when the task is assigned, b) the execution time of the task, c) the time for driving to the charing station, and d) the charging time.

The performance of the system can be seen in following figures. In Figure \ref{fig:delivery_time} we compare the average package delivery time for each number of robots on both maps. Figures \ref{fig:drive_pickup_dropoff}, \ref{fig:delivery_time2} and \ref{fig:trans_charg_idle} display the relative average time split for each state of the robot.

An important evaluation goal was to determinate the optimal amount of robots for each map. Figure \ref{fig:time_to_finish_40_tasks} shows the average time to finish 40 tasks and Figure \ref{fig:energy_consumption} presents the energy consumption of the system for 40 tasks for each number of robots.

\subsection{Evaluation Results and Interpretation}
\label{subsec:evaluation_results_interpretation}
The Figure \ref{fig:delivery_time} shows that the average package delivery time increases with an increasing number of robots. Initially, for one robot, a task can be executed as fast as possible, without any obstacles in between. As more robots participate in the simulation, robots need to avoid paths of other robots. This increases the coordination overhead which can lead to congestions. From robot 4 onwards, the package delivery time increases quite linearly with the increasing number of robots. A similar behaviour is observed for Map 2. However, the effect is quite distinct. By increasing the amount of trays, the density of the map is further increased, making it more difficult for the robots to navigate. 
%
...
In Figure \ref{fig:time_to_finish_40_tasks} we can see the positive effect of adding robots. For Map 1 the total time to finish 40 tasks decreases by more than 60 \% when we use 4 robots instead of 1. This certifies the goal of the system that the multi-robot collaboration significantly improves the performance of the system. Furthermore, we can see the performance improvement is just marginal from 8 robots onwards. As the number of robots increases and the available space remains the same, robots cancel the positive effect of the collaboration with the negative effect of having more coordination overhead. For the optimal average time to finish all tasks, 8 (for Map 2) or 10 (for Map 1) robots are preferred.


The Figures \ref{fig:drive_pickup_dropoff} and \ref{fig:delivery_time2} shed light on another aspect of the simulation: the bottleneck. Here, the time the task is in a queue is the waiting time. In Map 1, when the number of robots is significantly higher than the amount of pickup and delivery trays (e.g., when there are more than 8 robots), the pickup and drop-off tasks take substantially more time. As the available trays for pickup and drop-off are limited (4 and 5 respectively), robots have to coordinate themselves to get a space in front of the trays. This situation is much better in Map 2, when we increase the pickup and drop-off trays, removing the bottleneck. The differences between the maps is clearly seen in Figure \ref{fig:trans_charg_idle}, especially with more than 6 robots. In the first map, the robots spend more time in idle and charging. In the second map, as more trays are available, they are more of the time ready to execute the task instead of waiting. Hence, for the optimum package delivery time with the lowest waiting time, 10 robots are preferred.

Increasing the number of robots increases the overall power consumption. But if the tasks are done in a shorter time, lower energy consumption can be achieved. This can be clearly seen in the energy consumption in Figure \ref{fig:energy_consumption}. For the least energy consumption, 2 robots are preferred.


Analysing all the above points, the suggestion for the optimal amount of robots on a map of this particular size and layout is 6. It strikes a good balance between the time taken to perform all tasks, the time spent in idling and the energy that is consumed. Also, it considers the law of \emph{diminishing returns}. Even if there is a slight improvement in the performance, it is probably not worth the additional costs for hardware and maintenance.
