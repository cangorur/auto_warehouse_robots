...
\begin{abstract}
%\boldmath
Autonomous warehouses play an important role in smart factory environments which are an essential element of "Industry 4.0". The goal is to optimize the carriage of goods within the warehouse using multi-robot collaboration and coordination. This work uses hybrid task planning to improve the task assignment decisions and distributing the computational effort between the central task planner and the decentralized agents. A decentralized path planning with timed reservations using a shared map ensures the coordination between all robots while moving. The results show that this approach provides reliable and quick transportations in the warehouse up to high density of robots. Due to the limited space an increasing amount of robots leads to congestions and a high coordination overhead at some point reducing the overall effectiveness. This point can be determined within our simulation to find the best suitable amount of robots for a given warehouse configuration.
\end{abstract}
