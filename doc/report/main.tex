...
\begin{abstract}
%\boldmath
Autonomous warehouses play an important role in smart factory environments representing the "Industry 4.0". The goal is to optimize the carriage of goods within the warehouse using multi-robot collaboration and coordination. Hybrid task planning is used to improve the task assignment decisions and distributing the computational effort between the central task planner and the decentralized agents. A decentralized task planning with timed reservations using a shared map ensures the coordination between all robots while moving. The results show that this approach provides reliable and quick transportations in the warehouse up to high density of robots. Due to the limited space an increasing amount of robots leads to congestions and an high coordination overhead at some point reducing the overall effectiveness. This point can be determined within our simulation to find the best suitable amount of robots for a given warehouse configuration.
\end{abstract}
