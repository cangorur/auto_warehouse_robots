...
tmux send-keys -t rosc.0 "cd ${homedir} && source devel/setup.bash && morse run auto_warehouse auto_factory.py" enter

tmux send-keys -t rosc.1 "sleep 4 && cd ${homedir} && source devel/setup.bash && roslaunch --screen auto_smart_factory full_system.launch $grep" enter
