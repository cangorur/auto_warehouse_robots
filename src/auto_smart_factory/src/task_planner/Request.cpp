	...
	for (const RobotCandidate &cand : robotCandidates) {
		...
		if (sourceTray->isValid() && targetTray->isValid()) {
			...
			if(!targetTray->setPackage(pkg)) {
				ROS_ERROR("[request %d] Could not set package information at target tray (id: %d, type: %d)!", status.id, pkg.id, pkg.type_id);
			} else {
				ROS_WARN("[request %d] Successfully set package at target tray (id: %d, type: %d)!", status.id, pkg.id, pkg.type_id);
			}
...
bool Request::getRobotCandidates(std::vector<RobotCandidate> &robotCandidates,
	...
	if (useBestETA) {
		...
		std::sort(robotCandidates.begin(), robotCandidates.end(),
			[](RobotCandidate first, RobotCandidate second) {return first.estimatedDuration < second.estimatedDuration;});
	} else {
		// shuffle randomly
