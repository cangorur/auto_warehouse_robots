...
boost::property_tree::ptree FactoryGateway::GetStateOfStorages(boost::property_tree::ptree message_pt) {
	...
	for(int i = 0; i < currentTrayStates.size(); i++) {
		if(currentTrayStates[i].id > 11 &&
		   currentTrayStates[i].id < 31) {//from number 8 to number 16 the trays are container input trays
...
boost::property_tree::ptree FactoryGateway::GetStateOfChargingStations(boost::property_tree::ptree message_pt) {
	...
	sort(occupiedIds_array.begin(),
	     occupiedIds_array.end()); //TODO: after the get charging stations service will be updated this wont be needed anymore! issue#8
	for(int i = 0; i < 8; i++) {
