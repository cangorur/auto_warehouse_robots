...
bool PackageGenerator::newPackageInputOnConveyor(auto_smart_factory::Tray tray, auto_smart_factory::Package package) {
	...
	if (!movePackage(4 - 0.15 , 0.80, 1 , package)) { //TODO: currently the locations are hard coded to move the pkg on conveyor which runs and makes it fall to input tray-01 !! this will be fixed according to the trays !
